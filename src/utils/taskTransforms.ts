...
export const transformTaskData = (taskData: any): Task => {
  console.log('Transforming task data:', taskData); // Debug log
  ...
  const transformed = {
    ...
    // Transform subtasks from JSON
    subtasks: Array.isArray(taskData.subtasks) ? taskData.subtasks.map((st: any) => ({
      text: st.text || '',
  ...
  console.log('Transformed task:', transformed); // Debug log
  return transformed;
};
